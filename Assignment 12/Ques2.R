sample = c(.70, .84, .58, .50, .55, .82, .59, .71, .72, .61, .62, .49,
...
est_XII <- function(para){
  ...
  y[1] = n1/c + sum(log(sample_XII)) - (k+1)*sum((sample_XII^c)*log(sample_XII)/(1+sample_XII^c))
  y[2] = n2/k - sum(log(1+sample_XII^c))
  return(y)
}
...
est_X <- function(para){
  ...
  e = exp(-(c*sample_X)^2)
  y[1] = 2*(n2/c - c*sum((sample_X)^2)) + 2*c*(k-1)*sum((sample_X^2)*e/(1-e))
...
mle_X <- nleqslv(xstr, est_X,method='Newton',jacobian=TRUE)
mle_XII <- nleqslv(xstr, est_XII,method='Newton',jacobian=TRUE)

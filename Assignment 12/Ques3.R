sample = c(0.080, 0.084, 0.102, 0.124, 0.326, 0.358, 0.412, 0.444, 0.456, 0.504, 0.498, 0.564, 0.648,
...
est3 <- function(para){
  ...
  eta <- para[2]
  y <- numeric(0)
  y[1] = n/beta + sum(sample) - eta*sum(exp(beta*sample-1)*(sample))
  y[2] = n/eta - sum(exp(beta*sample-1))
  return(y)
}
